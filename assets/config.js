export const MAPBOX_ACCESS_TOKEN = "pk.eyJ1IjoieW9uaWxlMSIsImEiOiJjbWpnN29ub24wdjlhM2ZzZHF3Y245ZXI2In0.RDRLwEtomjDyCyXhycWtSQ";

export const DATA_URL = "./data/trail.geojson";
export const BICYCLE_DATA_URL = "./data/trail-bike.geojson";
export const POI_DATA_URL = "./data/poi.geojson";
export const GPX_URL  = "./data/trail.gpx";
export const FIRE_ZONE_INFO_URL = "https://www.idf.il/";

export const POI_ICON_URLS = {
  water_source: "./assets/icons/poi-water.svg",
  spring: "./assets/icons/poi-spring.svg",
  container: "./assets/icons/poi-container.svg"
};

export const SEG_COLORS = {
  1: "#6F7D4E",
  2: "#606C38",
  3: "#BC6C25",
  4: "#DDA25F",
  5: "#4F5D2F",
  6: "#8C4A1E",
};
export const SEG_OUTLINE = "#FEFAE1";
export const FALLBACK_COLOR = "#606C38";
export const SEG_COLORS_BIKE = {
  1: "#2F6B8A",
  2: "#3B82B4",
  3: "#1F8A70",
  4: "#4C9F70",
  5: "#2563A6",
  6: "#1D4E89",
};
export const SEG_OUTLINE_BIKE = "#E0F2FE";
export const FALLBACK_COLOR_BIKE = "#3B82B4";
export const HIGHLIGHT_COLOR = "#283518";
export const HIGHLIGHT_COLOR_BIKE = "#0B3D5C";
export const POI_COLOR = "#283518";
export const POI_STROKE = "#FEFAE1";
export const POI_COLOR_BIKE = "#0B3D5C";
export const POI_STROKE_BIKE = "#E0F2FE";

export const LOAD_TIMEOUT_MS = 15000;

export const DEFAULT_VIEW = {
  center: [35.5, 32.2],
  zoom: 8,
};
